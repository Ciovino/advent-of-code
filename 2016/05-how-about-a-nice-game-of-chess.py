# ---------------------------------------------------------------------
# Advent of Code 2016 - Day 05 - How About A Nice Game Of Chess?
# Problem: See ./2016/05-how-about-a-nice-game-of-chess-description.md for full details
# Author: Ciovino
# Template Version: v2.0
# ---------------------------------------------------------------------
import os
import argparse
import time

# Useful imports
import re
from collections import defaultdict, Counter, deque
from itertools import combinations, permutations, product
from math import gcd, lcm, ceil, floor
import hashlib

INPUT_FILE = os.path.join('data', '2016-05.in')
TEST_FILE = os.path.join('data', 'test.in')
VERBOSE = False

def log(*args, **kwargs):
    if VERBOSE: # Print only if VERBOSE is enabled
        print(*args, **kwargs)

def get_args() -> dict:
    parser = argparse.ArgumentParser(description="Solution script for 05/2016 Advent of Code.")
    parser.add_argument('-t', '--test', action='store_true',  help=f"Run the script using the test file ({TEST_FILE})")
    parser.add_argument('-v', '--verbose', action='store_true', help="Enable verbose output.")
    return parser.parse_args()

def find_numbers(text):
    return [int(n) for n in re.findall(r'-?\d+', text)]

def parse_input(file_name):
    with open(file_name, 'r') as f:
        data = f.read().strip()
    return data

# --- SOLVE ---
def solve_part1(room_id: str):
    """Solution for Part 1."""
    password = ['_']*8
    idx = 0
    c_index = 0
    while password.count('_') > 0:
        md5_hash = hashlib.md5(f"{room_id}{idx}".encode('utf-8')).hexdigest()
        if md5_hash[:5] == '00000':
            log(f"Found md5('{room_id}{idx}')='{md5_hash[:7]}...': ", end='')
            password[c_index] = md5_hash[5]
            c_index += 1
            log(''.join(password))
        idx += 1
    return ''.join(password)

def solve_part2(room_id: str):
    """Solution for Part 2."""
    password = ['_']*8
    idx = 0
    
    while password.count('_') > 0:
        md5_hash = hashlib.md5(f"{room_id}{idx}".encode('utf-8')).hexdigest()
        if md5_hash[:5] == '00000':
            log(f"Found md5('{room_id}{idx}')='{md5_hash[:7]}...': ", end='')
            try:
                position = int(md5_hash[5])
                # Update only if not set
                if password[position] == '_':
                    password[position] = md5_hash[6]
                log(''.join(password))
            except ValueError:
                log(f"Not valid position ('{md5_hash[5]}' not an integer)")
            except IndexError:
                log(f"Not valid position ('{md5_hash[5]}' out of bounds)")                
            
        idx += 1
    return ''.join(password)

if __name__ == '__main__':
    args = get_args()
    if args.test:
        if not os.path.exists(TEST_FILE):
            print(f"ERROR: Test file '{TEST_FILE}' not found.")
            exit(1)
        use_file = TEST_FILE
    else:
        use_file = INPUT_FILE
    VERBOSE = args.verbose
    
    # Parsing
    start_time = time.time()
    data = parse_input(use_file)
    log(f"Input parsed in {time.time()-start_time:.4f}s")
    
    # Part 1
    start_time = time.time()
    sol1 = solve_part1(data)
    log(f"Part 1: {sol1}, took {time.time()-start_time:.4f}s")
    
    # Part 2
    start_time = time.time()
    sol2 = solve_part2(data)
    log(f"Part 2: {sol2}, took {time.time()-start_time:.4f}s")

    # --- PRINT SOLUTIONS ---
    print(f"AOC_SOL_1={sol1}")
    print(f"AOC_SOL_2={sol2}")
