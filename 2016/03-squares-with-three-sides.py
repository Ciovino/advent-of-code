...
# Problem: See ./2016/03-squares-with-three-sides-description.md for full details
# Author: Ciovino
...
INPUT_FILE = os.path.join('data', '2016-03.in')
...
def get_args() -> dict:
    parser = argparse.ArgumentParser(description="Solution script for 03/2016 Advent of Code.")
...
def parse_input(file_name) -> list[tuple[int, int, int]]:
    with open(file_name, 'r') as f:
        data: list[tuple[int, int, int]] = [find_numbers(line) for line in f]
    return data
...
def solve_part1(triangles: list[tuple[int, int, int]]):
    """Solution for Part 1."""
    return sum([is_valid_triangle(t) for t in triangles])

def solve_part2(triangles: list[tuple[int, int, int]]):
    """Solution for Part 2."""
    triangles_by_columns: list[tuple[int, int, int]] = []
    for i in range(0, len(triangles), 3):
        t1, t2, t3 = triangles[i:i+3]
        triangles_by_columns += list(zip(t1, t2, t3))
    return sum([is_valid_triangle(t) for t in triangles_by_columns])
